...
- 校验初始化产物已使用 `stream_tag` / `stream_tag = ""`，不再包含旧的 `stream =` 配置字段
...
2. 检查 `tmp-work/conf`、`tmp-work/models`、`tmp-work/topology`、`tmp-work/test` 中的 `.toml/.wfs` 已对齐 `stream_tag` 配置
3. 进入 `tmp-work/`
4. `./smoke.sh` — 三步 batch 验证:
   - `wfgen lint` — 场景语法校验
   - `wfgen gen` — 从场景生成 NDJSON 演示数据（30000 条事件）
   - `wfusion batch` — batch 模式回放数据 + 产出 654 条 port_scan 告警
5. `./test_run.sh 30s` — TCP daemon 联调:
