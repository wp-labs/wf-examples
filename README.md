...
运维控制面，到 NEXMark 基准对标与规则规模/内存稳定性验证，每个 case 自带规则、场景、
配置与一键脚本，可直接复跑并产出数据。
...
| [`performance/rule_scale_test/`](performance/rule_scale_test/README.md) | 合成大规则集规模测试（376 规则高压吞吐；规则家族成本/墙梯/二分；run.sh 内置 #18 object 驱逐门禁） | `./run.sh` `./diag.sh` `./sweep.sh` |
