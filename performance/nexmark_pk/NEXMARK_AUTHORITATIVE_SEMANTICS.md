...
> **本文件只记录权威定义，不含任何对齐/评审结论。** 对齐情况见同目录 `REVIEW_FLINK_SEMANTIC_ALIGNMENT.md`。
...
- 我们自己的实现位于 `wf-examples/performance/nexmark_pk/models/queries/qN.wfl`，其与上述权威语义的对齐情况**不在本文件讨论范围**，见 `REVIEW_FLINK_SEMANTIC_ALIGNMENT.md`。
