...
## 6b. Q1~Q22 10M 对拍全量基线（2026-08-23 复跑：q5/q11 尾部收口修复后）
...
| Q4 | 5 | 5 | ✅ 一致 |
| Q5 | 529 | 529 | ✅ 一致（08-23 修复：尾部未完整窗口不发射；原 532，多 3 条即尾部 992/994/996s 窗口 w_end>1000s） |
| Q6 | 8,731,356 | 8,731,356 | ✅ 一致（修复后） |
| Q7 | 100 | 100 | ✅ 一致 |
| Q8 | 82,446 | 82,446 | ✅ 一致 |
| Q9 | 557,204 | 557,204 | ✅ 一致（修复后） |
| Q10 | 9,200,000 | 9,200,000 | ✅ 一致 |
| Q11 | 197,095 | 197,095 | ✅ 一致（08-23 修复：session 尾部收口语义对齐——① close_all 未超时会话不发射；② scan_timeouts 不叠加墙钟；③ flush 按窗口全局末尾补扫，分片 shard 水位不再误判；原 197,299，多 204≈0.1%） |
...
> 多输出 3 条（尾部 992/994/996s 窗口 w_end=1002/1004/1006 > 1000s）。

> 2026-08-23 q11 修复（三处同源，`close_all`/`scan_timeouts`/`flush` 收口语义对齐 oracle）：
> ① `close_all` 对 session 尾部未超时会话（`last_event+gap >` 最终事件时间）释放实例但不发射；
> ② `scan_timeouts` 对 session 不叠加墙钟（gap = 事件时间间隔，replay 对拍依赖事件时间序）；
> ③ `flush` 先用窗口 raw `max_event_time`（全局末尾，新增 API）补扫一次再 `close_all`——
> 分片下 shard 状态机水位落后全局末尾，尾部完整会话/窗口被误判未完整而跳过（10M 少 1 条）。
> 修复前引擎多 204/197,095≈0.1%（快速 replay 末尾 10s gap 内会话被强制收口）。
> 注：oracle eos 水位仍为 `count×100µs`（覆盖数据末尾的 slice 收口语义，与 q5/q11 窗口边界正交）。
