...
| q18 | 39,730MB | **40,787/37,755MB** | 🔴 真实超标 | ⚠ M-18 |
| q19 | 23,492MB | 7,066MB | ✅ 批内污染 | — |
| **q22** | **27,048MB** | 3,950MB / **24,490MB** / 8,938MB | 🔴 M-22 真实波动 4-24.5G → ✅ **已解决（4.0-5.4G）** |
...
- RSS ~38-41GB（**单独 100M 确认**：40.8G / 37.8G，批内 39.7G）；EPS 15.5-15.9M；evict=2272。
- 机制：stats 1d group by (bidder,auction)（键基数最大 2935 万）× 4 个 last 度量保留整行；
  状态 9.8G（键数=数据特征不可减，见 wp-reactor notes q18 文档）+ 窗口 3.6G + parse 2GB
  + allocator 保留。**已做**：StatsAccum 紧凑化（208B→24B）、列式直写 close（60G→33G）、
  分块 flush；剩余 ~28G 待归因（分片开销/allocator 保留）。
- 修复方向：规则级。
- 状态：未解决。
