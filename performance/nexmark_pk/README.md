# nexmark_pk — NEXMark 基准查询吞吐（对齐 Flink 官方基线）

与 Flink 对齐的 PK case：**同一份权威基准数据（NEXMark）+ 同一批查询（Q1/Q2/Q4/Q5/Q7）**，
跑我们引擎的实测吞吐，对照 Flink 官方发布的 Nexmark 基线。这是**数据+查询**对齐的对比
（区别于 flink_pk 用的 PatternStudio 锚——后者数据集未公开）。

## 数据（NEXMark 事件模型）

**原生 Rust 生成**：`wfgen gen-nexmark <count> [--seed N]`（warp-fusion 的 wfgen 子命令，
不依赖 Python）。`scripts/gen_nexmark.py` 保留为参考实现（算法一致）。对齐标准 NEXMark schema：

| 流 | 占比 | 字段 |
|---|---|---|
| person_events | 2% | id/name/email/city/state/dateTime |
| auction_events | 6% | id/itemName/description/initialBid/reserve/dateTime/expires/seller/category/extra |
| bid_events | 92% | auction/bidder/price/channel/url/dateTime/extra |

事件时间覆盖 ~30 分钟（3 个 10m 窗口）；hot 分布（50% hot auction / 25% hot bidder /
25% hot seller）。重新生成：`wfgen gen-nexmark 200000 > data/burst.jsonl`。

## 查询（12 条 WFL 规则，10m 窗口）

`models/rules/nexmark.wfl` 实现 NEXMark 查询子集：
- **Q1** pass-through 基线（bid 计数）
- **Q2** 按 auction id 过滤（stateless filter）
- **Q4** 每 category 10m 窗口均价（窗口聚合）
- **Q5** 每 auction bid 计数（Top-N 的计数面）
- **Q7** 每 auction 10m 窗口最高出价（滑动窗口 MAX）

> Q3（person+auction 按 seller=id join）需跨字段 join key，当前 match-key 模型不支持，
> 列为后续项。单查询隔离文件在 `models/queries/{q1,q2,q4,q5,q7}.wfl`。

## 实测（200000 事件，release，单 Mac，2026-08-11）

EPS 用 send 墙钟计时（单连接流式）。**默认输出为 blackhole sink**（丢弃、只计数——
对齐 Flink Nexmark 的 discarding sink 口径）；文件 sink（含落盘）作保守口径列后表。
组合（12 规则）与单查询隔离各跑。

### blackhole 输出（= Flink 口径，纯处理吞吐）

| 负载 | 送达 | EPS | 驱逐 |
|---|---|---|---|
| 组合 Q1+Q2+Q4+Q5+Q7 | 200000 | **~478k** | 0 |
| 仅 Q1 | 200000 | ~485k | 0 |
| 仅 Q2 | 200000 | ~499k | 0 |
| 仅 Q4 | 200000 | **~496k** | 0 |
| 仅 Q5 | 200000 | **~484k** | 0 |
| 仅 Q7 | 200000 | **~482k** | 0 |

### 文件 sink 口径（保守，含落盘，batch 1024/1s）

| 负载 | EPS | 告警 |
|---|---|---|
| 组合 | ~453k | ~271k |
| 仅 Q4 | ~469k | 12k |
| 仅 Q5 | ~481k | 3.6k |
| 仅 Q7 | ~455k | 34k |

### CPU 利用率（1M 事件，16 核 Mac）

| 指标 | 值 |
|---|---|
| 引擎 CPU 峰值 | ~905%（≈9 核） |
| 引擎 CPU 均值 | ~479%（≈5 核） |
| 机器利用率 | 均值 ~30%、峰值 ~56% |

**引擎未吃满**（稳态 ~5 核 / 16 核），有 ~3 倍余量。

## 对 Flink 官方 Nexmark 基线（同输出口径：都丢弃）

来源：[Alibaba Nexmark 白皮书](https://help.aliyun.com/en/flink/realtime-flink/support/nexmark-performance-testing)
——开源 Flink 1.20.4，**8 CU（≈8 vCPU + 32GB）**，100M 记录/查询，RPS=records/sec（输入事件/秒）。

| 查询 | 我们（1 Mac，blackhole） | Flink（8 CU，discard） | 倍率 |
|---|---|---|---|
| Q4 窗口均价 | **~496k** | 180,693 | ~2.7× |
| Q5 计数 | **~484k** | 273,496 | ~1.8× |
| Q7 窗口 MAX | **~482k** | 79,526 | ~6.1× |

## 诚实边界

1. **数据/查询/输出已对齐**：同一 NEXMark 事件模型 + 同一查询 + 同为丢弃输出（blackhole），
   units 均为输入事件/秒——与 flink_pk（PatternStudio 锚，数据集未公开）本质不同。
2. **硬件/规模不同**：我们 1 台 16 核 M 系 Mac、200k–1M 事件；Flink 基线 8 vCPU 云主机、
   100M 记录。单机对 8 vCPU 仍领先 ~1.8–6.1×，且引擎稳态只用 ~5 核（未吃满）——
   差距在引擎架构（#19 共享解析 + 单遍规则评估）而非资源。
3. **Q4 用 initialBid 代 closing price**；Q5/Q7 为阈值版（非完整 Top-N 输出）。
4. **Q3 join 未实现**——引擎跨字段 join key 是当前模型缺口。

## 前提

- `wfusion` / `wfgen` 在 PATH（或用 `WFUSION=/path WFGEN=/path`），wfgen 需含
  `--chunk/--rate-ms` 单连接流式支持（warp-fusion 84333b5）。
- `nc`、`python3`；端口 9800 空闲。
