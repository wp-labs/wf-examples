...
> ⚠️ **口径修订（2026-08-16）**：下表 wfusion EPS 为**旧 ingress 计时口径，已作废**
> （bench.sh 已改 append_total 口径，见 README 测量纪律）。方向性结论（复杂窗口查询
> 大幅领先、简单查询与 VVR 相当）仍成立，但**倍率幅度须按新口径重跑刷新**后再对外引用。
> 新口径当前锚点：Q1 100M = **5.02M/s @3.29GB**（vs OSS 2.9× / vs VVR 1.15×）；
> Q2-Q9 30M 单轮参考值见 README。
