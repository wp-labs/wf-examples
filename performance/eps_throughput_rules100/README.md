...
- RSS 为送达后继续采样 8s 的平台期峰值：实例存活至 2m 窗口关闭，送达即杀进程会
  严重低估（早期测得 0.7GB / 14GB，实际 3.4GB / 19GB）。
- EPS 同机多跑有波动（机器负载相关），本表为代表值；三次运行驱逐告警全为 0，#18 门禁通过。
- 300 规则 vs 20 规则（`eps_throughput_obj`）：EPS 量级相当（~120-190k vs ~117k）——
  规则数翻 15 倍吞吐不降，因事件解析共享（#19）。
- flood 模式成本在**实例内存**：100000 独立 sip × per-sip 规则 = 千万级实例，RSS ~19GB；
