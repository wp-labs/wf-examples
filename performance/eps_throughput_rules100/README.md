...
> distinct 的吞吐并不慢（~95k，全送达）；成本在**实例内存**：100000 独立 sip ×
> ~90 条 conn 规则 = **~900 万实例**，正好对齐规则 `max_instances=100000` 封顶
> （每个 sip 都拿到实例，无 throttle 截断）。这是与 #19 不同的扩展维度：
> **实例数 × 规则数**（每实体每规则一份状态）。pool 模式 1000 sip → ~10 万实例，
> RSS 才 2.5GB。
...
distinct 模式在一个 2 分钟窗口内出现 **100000 个独立源 IP（sip）**。这对应什么现实场景？

| 部署规模 | 单窗口内独立 sip | 100 规则实例数 |
|---|---|---|
| 企业内网（1-10 万内部 IP） | 几千 | ~50 万 |
| **大型企业 / 校园网** | **1-10 万** | **~900 万（本场景）** |
...
- **distinct 模式（100000 唯一 sip）代表"大型企业 / ISP 边缘"的高负载**：每个源 IP
  只发一两条连接，用于压测**实例 churn 上限**。取 100000 而非更大的值，是因为：
  1. 与规则 `max_instances=100000` 封顶对齐——每个 sip 都拿到实例，压测完整的高基数
     实例集，不被封顶截断。
  2. 落在"大型企业 / ISP 边缘"这个真实高负载区间，而非仅攻击风暴的极端。
- **对生产的意义**：实例内存 = 窗口内独立实体 × 规则数。企业典型负载（几千实体 ×
  100 规则）完全无压力；大型企业 / ISP 边缘（10 万实体 × 100 规则 = ~900 万实例）
  触及内存边界——这正是 distinct 模式要暴露的扩展瓶颈。

### 内存由 max_instances 封顶决定，不由 sip 数决定

实例数 = `min(唯一 sip, max_instances/规则) × 规则数`。一旦唯一 sip 数 ≥ 封顶，
**加更多 sip 不会增加内存**（多余的被 throttle 丢弃）：

| distinct 唯一 conn sip | 每规则实例数 | 总实例 | RSS |
|---|---|---|---|
| 200k（旧） | 100k（被封顶截断一半） | ~900 万 | ~11.8GB |
| **100k（当前）** | **100k（正好到封顶）** | **~900 万** | **~11.3GB** |
| 50k（对照） | 50k（低于封顶） | ~450 万 | ~7.2GB |

所以 distinct 改 100k 不是为了降内存（封顶下两者都是 ~900 万实例、RSS 相当），
而是让压测**语义完整**：每个 sip 都拿到实例、无 throttle 截断，验证的是**规则允许的
最大实例数下的满负荷行为**。要真正降低 distinct 内存，需要把 sip 数或 max_instances
降到封顶以下——但那就不是高基数压测了。
