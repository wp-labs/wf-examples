...
| `burst` distinct | 200000 | **~85k** | **~15GB**（2000 万实例） |

> distinct 的吞吐并不慢（~85k，全送达）；真正的问题是**实例内存爆炸**：
> 200000 独立 sip × 100 规则 = **2000 万个实例**，每个 ~750B 状态（scope key +
> field_values HashMap 等）→ RSS ~15GB。这是与 #19 不同的扩展维度：**实例数 ×
> 规则数**（每实体每规则一份状态）。pool 模式 1000 sip → 10 万实例，RSS 才 2.8GB。
