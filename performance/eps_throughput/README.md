...
> ⚠ **本页数据为历史数据，待复测**（README 最后更新 **2026-08-10**）。
> 该测量**早于 2026-08-16 落地的批式向量化等重大吞吐优化**，也早于 2026-08-27 的 NEXMark 权威跑批，
> 已低估当前引擎能力。**复测前请勿将本页数字作为容量承诺或对外引用。**
> 若必须引用，须同时标注测量日期与口径（macOS / 4 并发 / 20 万事件 / 多规则真实告警落 sink）。
> 复测建议：对齐 NEXMark 口径（Linux 8 核）、补 32 核扩展度、并分别记录"告警落 sink"与"blackhole"两种汇。
