#!/usr/bin/env bash
# flink_pk — PK-Flink 专用：250 条 CEP 风格规则（seq/close/multi/pipe/guard）高压吞吐
#
# 250 条规则（scripts/gen_rules.py 生成）以模式为主：seq 2/3 步 120、close 40、
# multi 36、pipeline 10、guard 44。对标 Flink CEP 锚点（PatternStudio：单节点
# 250 pattern ~47.9k EPS）。本 case 只测引擎在模式密集规则下的吞吐，不做 #18 门禁。
#
# 用法:
...
if [ "$MODE" = "peak" ]; then
  ...
  "$WFGEN" send --scenario scenarios/pk.wfg --input data/burst.jsonl \
